...
import { RefreshCw, Plus, X, Target } from 'lucide-react'
...
function AddBudgetModal({ month, onClose, onSave }) {
    const [form, setForm] = useState({ month, category: '', amount: '' })
    function set(k, v) { setForm(f => ({ ...f, [k]: v })) }
    async function submit(e) {
        e.preventDefault()
        await api.setBudget({ ...form, amount: parseFloat(form.amount) })
        onSave()
    }
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="glass-card w-full max-w-sm p-6 animate-slide-up">
                <div className="flex items-center justify-between mb-5">
                    <h3 className="font-bold text-slate-100">Definir Orçamento</h3>
                    <button onClick={onClose} className="btn-ghost p-1.5"><X size={16} /></button>
                </div>
                <form onSubmit={submit} className="space-y-3">
                    <div><label className="label block mb-1">Mês</label>
                        <input className="input-field" readOnly value={form.month} /></div>
...
export default function Budgets() {
    ...
    useEffect(() => {
        api.defaultMonth().then(({ budgets_month }) => setMonth(budgets_month)).catch(console.error)
    }, [])

    const load = useCallback(async () => {
        if (!month) return
        setLoading(true)
        try { setBudgets(await api.budgets(month)) } finally { setLoading(false) }
    }, [month])
        ...
        <div className="space-y-6 animate-fade-in">
            ...
            {!loading && budgets.length === 0 && (
                <div className="glass-card p-12 text-center">
                    ...
                    <div className="text-slate-500">Nenhum orçamento definido para {month}</div>
                    <button onClick={() => setShowModal(true)} className="btn-primary mt-4">Definir orçamento</button>
                </div>
            )}

            {!loading && budgets.length > 0 && (
                <div className="glass-card overflow-hidden">
                    {budgets.map(b => <BudgetRow key={b.category} item={b} />)}
                </div>
            )}

            {showModal && <AddBudgetModal month={month} onClose={() => setShowModal(false)} onSave={() => { setShowModal(false); load() }} />}
        </div>
    )
}
